...
export default function Input({ password, placeholder, value, setInput, elementFocus, statusError }: InputProps) {
  ...
  useEffect(() => {
    if(elementFocus){
    const ele = elemento.current;
    ele?.focus()}
  }, []);

  return (
    <>
      <StyledInput  ref={elemento} onChange={(e)=>{setInput(e.target.value)}} placeholder={placeholder} type={displayContent ? "text" : "password"} value={value} statusError={statusError}/>
      {password && (
